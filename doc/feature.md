...
✅ **Balance & polish pass (2026-07-05)** — fix 2 rough edge từ đợt 3/4:
  1. **Prestige mất khi nhặt buff:** `EffectiveStats.withBuffs` re-clamp về cap gốc (3.0/4.0) → player đã prestige nhặt bất kỳ roguelike buff nào bị kéo tụt, mất reward. Fix: lưu `prestigeMul` vào EffectiveStats; withBuffs tách prestige → buff lên base (cap gốc) → nhân lại prestige (cap rộng 6.0/8.0), mirror đúng `compute`. `prestigeMul=1` ⇒ hành vi cũ nguyên vẹn. Test: `EffectiveStatsTest` +2 regression.
  2. **Heal cap không đồng nhất (under-heal):** 3 site (bánh mì/LIFESTEAL/REGEN) hardcode `maxHp=1000` → ship hpMul>1 (gần như mọi run) không hồi được phần hp trên 1000; 2 site (drone HEAL/REPAIR) dùng `(1000×hpMul).coerceAtLeast(1)` thiếu cap trên. Thêm instance #3: BOSS_RUSH `setHp(1000)` giữa 2 boss. Thống nhất cả 6 về `initialShipHp` (single source, `coerceIn(100,3500)` gồm legendary bonus).
  3. **updateHp overheal (Finding #4):** `updateHp` (HEALING_AURA +5/s, REGEN booster, vampire, QUICK_HEAL) heal KHÔNG cap trên → overheal vượt max hp (trái intent `regenTick`/`healCapped`). Thêm `maxHpProvider: ()->Int` vào ShipController (GameState truyền `{ initialShipHp }`), clamp heal trong updateHp; default `Int.MAX_VALUE` giữ hành vi cũ cho test. Test: `Wave11aShipControllerBehaviorTest` +2 (`applyVampireHeal` cap / uncapped-default).
  - **Device eyeball (emulator Pixel 10 Pro XL):** logcat xác nhận max hp thật = 1080 (Vừa) / 1296 (Aura) / 1542 (Dễ) — đều >1000, và `EffectiveStats(... prestigeMul=1.08)` áp dụng đúng. Ảnh chụp khoảnh khắc hp vượt 1000 qua heal không canh được (ship chết trong bullet-hell qua blind-tap adb) — giới hạn tự động hoá, không phải lỗi fix; đã cover bằng unit test.
