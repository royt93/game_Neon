...
- 🟡 **Đạn trào phúng MỚI (theo batch).** Mỗi đạn đụng ~8 `when` exhaustive (buildOneLaser, collision, BulletTypeColorMap, LaserCanvas, DialogLoadoutPicker ×4, InfoScreen ×2, ShopScreen) + pin count/name-set.
  ...
  - 📋 **Batch 3+ (còn ~5):** Trà Sữa (nổ trân châu), Nước Mắm (ăn mòn DoT), Dép Lào (boomerang — cần class mới), Giấy Phạt (homing), Mã QR (debuff).
  - ⚠️ **AUDIT shape/skill (user hỏi 30-31/05): yêu cầu CỨNG = đạn/boss phải skill+shape tách bạch, không trùng.**
    ...
    - 📋 **Slice 4c — InfoScreen/loadout preview riêng** cho 6 đạn (hiện preview batch2 còn reuse).
...
*Khoáng (Wave 14b):* 📋 thưởng khoáng achievement / daily login / mốc stage.
...
*Boss — SCOPE (đã audit framework):* hệ boss **data-driven** sẵn: 27 `BossKind` (đã có nhiều trào phúng: Cộng Sản Lên Ngôi, Tư Bản Bóc Lột, Gà Mái Dầu, Trâu Hung Hãn, Cô Gái Sexy, Bạch Long, Tháp Tinh Quỷ, Đôi Cầu Hư Vô…) + `MidBossType` (12+ variant, baseHp/drawable/bossKind) + `Chapter.midBossTypes` list + `MidBoss` pattern `when(variant)` có `else ->` mặc định. **Thêm 1 boss mới = BossKind entry + MidBossType variant + chapter list + InfoScreen Bách Khoa + (bespoke attack pattern) + cập nhật ~6 pin test (MetaProgressionKeysTest BossKind count, BossKindTest, EnemyFactoryBossKindTest, ChapterMidBossWireTest, StoryRegistryBossTauntTest) + StoryRegistry taunt + EnemyCanvas drawBoss<Kind>.** → mỗi boss đụng nhiều file; làm batch nhỏ/wave + verify.
...
- 🟡 **14a — làm dày Shop (đang làm, 3 round):**
  ...
  - 📋 **Round 2 (consumables):** thêm 6 gói tiêu hao (mua→run kế→hết) — pick: **x2 Khoáng / Khiên khởi đầu / x2 Điểm / Giữ Combo** + đề xuất mới: **Nam châm XL / Bắn nhanh**. Catalog + consumer run-start trong GameState (mineralMult, seed shield, scoreMult, combo-no-reset, magnetMult, fireRate).
  - 📋 **Round 3 (skin mới):** thêm 2-3 hào quang màu mới (vd Xanh lá/Cam/Trắng) — ShipSkin + ShopItem + i18n.
- 📋 **14b (#6 — tăng thu nhập Khoáng, pick "thêm nguồn mới"):** thưởng khoáng theo achievement / daily login / mốc stage (gói x2 Khoáng ở 14a round 2 đã cover 1 phần).
...
### 9a Enemy variant expansion 🟡 PARTIAL (R73-R78 + R81/R82 — 10/15 mới)
